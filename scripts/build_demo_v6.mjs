...
const input = `${root}artifacts/ClassTrek-demo-v5-participation.mp4`;
const exchange = `${root}artifacts/video-v6/raw/journey-exchange.webm`;
const overlays = `${root}artifacts/demo-role-overlays-v6`;
const narration = `${root}artifacts/video-v6/audio/trek-exchange.aiff`;
const soundBed = `${root}artifacts/ClassTrek-original-sound-bed.wav`;
const output = `${root}artifacts/ClassTrek-demo-v6-exchange.mp4`;
...
const filter = [
  ...
  "[0:a]atrim=start=0:end=9.800,asetpts=PTS-STARTPTS[audioPre]",
  "[8:a]aresample=48000,volume=1.15,adelay=400," +
    "apad=pad_dur=16.433,atrim=duration=16.433,asetpts=PTS-STARTPTS[voice]",
  "[9:a]atrim=start=9.800:end=26.233,asetpts=PTS-STARTPTS," +
    "volume=0.22[bed]",
  "[voice][bed]amix=inputs=2:duration=longest:normalize=0," +
    "atrim=duration=16.433[audioMiddle]",
  "[0:a]atrim=start=26.233,asetpts=PTS-STARTPTS[audioPost]",
  "[audioPre][audioMiddle][audioPost]concat=n=3:v=0:a=1[audio]",
].join(";");
